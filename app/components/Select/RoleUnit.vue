...
const props = defineProps({
  ...
  label: { type: String, default: "Nhóm quyền đơn vị:" },
  name: { type: String, default: "unit_role_group" },
...
const fetchRoleGroups = async (searchTerm = "") => {
  loading.value = true;
  try {
    ...
    const { data, error } = await unitRoleGroup.get({ params });
    ...
    options.value = [];
    message.error(err?.message || "Không thể tải danh sách Nhóm quyền");
  } finally {
    loading.value = false;
  }
};
