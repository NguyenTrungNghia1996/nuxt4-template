  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50">
    <div class="mx-auto max-w-5xl space-y-8 px-4 py-10">
      <header class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="space-y-1">
          <div
            class="inline-flex items-center gap-2 rounded-full bg-teal-100 px-3 py-1 text-xs font-semibold text-teal-700">
          ...
          <p class="max-w-2xl text-sm text-gray-600">
            Thử component SelectRoleAdmin (lấy dữ liệu từ <code>/superadmin_role_groups</code>,
      ...
      <section class="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
        <article class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
          <div class="flex items-start justify-between gap-3">
            <div>
              ...
              <p class="text-sm text-gray-600">
                Bật/tắt các option để xem layout và hành vi; cần token hợp lệ để gọi API nhóm quyền.
              </p>
            </div>
            ...
            <div class="space-y-3 rounded-xl border border-gray-100 bg-gray-50 p-4">
              ...
              <ClientOnly>
                <a-form layout="vertical" :model="{ singleRole }">
                  <SelectRoleAdmin
                    v-model="singleRole"
                    :show-navigation-buttons="controls.showNavigation"
                    :auto-select-first="controls.autoSelectFirst"
                    :disabled="controls.disabled"
                    placeholder="Tìm hoặc chọn nhóm quyền"
                  />
                </a-form>
              ...
              <div class="rounded-lg bg-white px-3 py-2 text-xs text-gray-600 ring-1 ring-gray-200">
                Tip: clear giá trị để reset search; bật auto-select-first sẽ chọn option đầu sau khi tải.
              </div>
            </div>
            ...
            <div class="space-y-3 rounded-xl border border-gray-100 bg-gray-50 p-4">
              ...
              <ClientOnly>
                <SelectRoleAdmin
                  ...
                  :auto-select-first="controls.autoSelectFirst"
                  :disabled="controls.disabled"
                  placeholder="Chọn nhiều nhóm quyền"
                />
              ...
              <div class="rounded-lg bg-white px-3 py-2 text-xs text-gray-600 ring-1 ring-gray-200">
                Dùng khi không cần a-form-item nhưng vẫn muốn label cùng hàng với select.
              </div>
            </div>
          </div>
          ...
          <div class="mt-4 flex flex-wrap gap-2 text-xs text-gray-700">
            <button
              class="inline-flex items-center gap-2 rounded-lg bg-teal-600 px-3 py-2 font-semibold text-white shadow-sm transition hover:bg-teal-700"
              @click="resetValues">
        ...
        <aside class="space-y-4">
          <div class="space-y-3 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
            <div class="flex items-center justify-between text-gray-900">
              ...
              <button
                class="inline-flex items-center gap-1 rounded-md bg-teal-50 px-2 py-1 text-[11px] font-semibold text-teal-700 ring-1 ring-teal-100 transition hover:bg-teal-100"
                @click="copyState">
...
{{ stateJson }}
          ...
          <div class="space-y-3 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
            <div class="flex items-center justify-between text-gray-900">
              ...
              <button
                class="inline-flex items-center gap-1 rounded-md bg-slate-900 px-2 py-1 text-[11px] font-semibold text-white transition hover:bg-slate-800"
                @click="copyUsage">
...
const singleRole = ref<string | number | null>(null);
