  ...
  <div class="min-h-full rounded bg-white p-4 shadow">
    ...
    <a-modal v-model:open="visible" :title="modalTitle" @cancel="handleCancel" :width="modalWidth" :bodyStyle="{ maxHeight: '70vh', overflowY: 'auto' }" destroy-on-close>
      <div class="grid grid-cols-1 gap-2 md:grid-cols-2">
        ...
        <PermissionEditor v-model="formState.permissions" scope="unit" />
      </div>
...
const handleOk = async () => {
  try {
    ...
    if (isEdit.value) {
      const { data, error } = await unitRoleGroup.put({ body: payload });
      ...
      const { data, error } = await unitRoleGroup.post({ body: createPayload });
      if (data.value?.status === "success") {
...
const deleteItem = async record => {
  ...
  deletingId.value = id;
  try {
    const { data, error } = await unitRoleGroup.delete({ params: { id: String(id) } });
    if (data.value?.status === "success") {
